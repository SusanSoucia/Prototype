...
import com.group45.backend.config.AppAuthProperties;
import com.group45.backend.exception.CustomException;
import com.group45.backend.model.RegistrationMode;
...
import com.group45.backend.utils.PasswordUtil;
...
public class UserService {
    ...
    private void validateRegistrationPolicy(String username, String inviteCode) {
        ...
        // Note: invite code validation removed in Prototype (no InviteCodeService)
    }
     ...
     * @param role 目标角色（ADMIN 或 LIBRARY_ADMIN）
     * @param creatorUsername 创建者的用户名（必须是已存在的管理员）
     * @throws CustomException 如果用户名已存在或创建者不是管理员，则抛出异常
     */
    public void createAdminUser(String username, String password, User.Role role, String creatorUsername) {
        // 验证创建者是否为管理员
        ...
        // 只有 ADMIN 可以创建 ADMIN
        if (role == User.Role.ADMIN && creator.getRole() != User.Role.ADMIN) {
            throw new CustomException("Only ADMIN can create ADMIN accounts", HttpStatus.FORBIDDEN);
        }

        // 不允许创建 USER 角色（USER 应通过注册接口创建）
        if (role == User.Role.USER) {
            throw new CustomException("Use register endpoint to create USER accounts", HttpStatus.BAD_REQUEST);
        }
    ...
    /**
     * 创建管理员用户（默认为 ADMIN 角色）。
     * @deprecated 请使用 {@link #createAdminUser(String, String, User.Role, String)} 明确指定角色
     */
    @Deprecated
    public void createAdminUser(String username, String password, String creatorUsername) {
        createAdminUser(username, password, User.Role.ADMIN, creatorUsername);
    }
     ...
     * @throws CustomException 如果用户名不存在、旧密码错误或新密码格式不正确
     */
    @Transactional
    ...
    public void assignOrgTagsToUser(Long userId, List<String> orgTags, String adminUsername) {
        ...
        // 标签驱动角色：有真实标签 → LIBRARY_ADMIN，无真实标签 → USER（不影响 ADMIN）
        Set<String> realTags = new HashSet<>(finalTags);
        realTags.remove(privateTagId); // 排除私人标签
        if (user.getRole() == User.Role.USER && !realTags.isEmpty()) {
            user.setRole(User.Role.LIBRARY_ADMIN);
        } else if (user.getRole() == User.Role.LIBRARY_ADMIN && realTags.isEmpty()) {
            user.setRole(User.Role.USER);
        }
    ...
    public Map<String, Object> getUserList(String keyword, String orgTag, Integer status, int page, int size) {
        ...
        List<Map<String, Object>> userList = userPage.getContent().stream()
                .map(user -> {
                    ...
                    // status: 0=ADMIN, 1=USER, 2=LIBRARY_ADMIN
                    int statusCode = switch (user.getRole()) {
                        case ADMIN -> 0;
                        case USER -> 1;
                        case LIBRARY_ADMIN -> 2;
                    };
                    userMap.put("status", statusCode);
    ...
    private boolean matchesUserListFilters(User user, String keyword, String orgTag, Integer status) {
        ...
        if (status != null) {
            User.Role targetRole = switch (status) {
                case 0 -> User.Role.ADMIN;
                case 1 -> User.Role.USER;
                case 2 -> User.Role.LIBRARY_ADMIN;
                default -> null;
            };
