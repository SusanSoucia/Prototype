...
import com.group45.backend.service.ConversationService;
import com.group45.backend.service.ModelProviderConfigService;
import com.group45.backend.service.RateLimitConfigService;
import com.group45.backend.service.UserService;
import com.group45.backend.utils.JwtUtils;
import com.group45.backend.utils.LogUtils;
import com.group45.backend.utils.MinioMigrationUtil;
...
public class AdminController {
    ...
    public ResponseEntity<?> getAllUsers(@RequestHeader("Authorization") String token) {
        ...
        try {
            ...
            User admin = validateAdminOrLibraryAdmin(adminUsername);
    ...
    public ResponseEntity<?> createAdminUser(
        ...
        try {
            User.Role targetRole = parseRole(request.role());
            userService.createAdminUser(request.username(), request.password(), targetRole, adminUsername);
            return ResponseEntity.ok(Map.of("code", 200, "message", "用户创建成功（角色: " + targetRole.name() + "）"));
        } catch (CustomException e) {
            LogUtils.logBusinessError("ADMIN_CREATE_ADMIN_USER", adminUsername, "创建用户失败: %s", e, e.getMessage());
            return ResponseEntity.status(e.getStatus()).body(Map.of("code", e.getStatus().value(), "message", e.getMessage()));
        } catch (Exception e) {
            LogUtils.logBusinessError("ADMIN_CREATE_ADMIN_USER", adminUsername, "创建用户异常: %s", e, e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("code", 500, "message", "创建用户失败: " + e.getMessage()));
        }
    }

    private User.Role parseRole(String role) {
        if (role == null || role.isBlank()) {
            return User.Role.ADMIN; // default to ADMIN for backward compatibility
        }
        try {
            return User.Role.valueOf(role.toUpperCase());
        } catch (IllegalArgumentException e) {
            throw new CustomException("Invalid role: " + role + ". Valid roles: ADMIN, LIBRARY_ADMIN", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<?> getAllOrganizationTags(@RequestHeader("Authorization") String token) {
        String adminUsername = jwtUtils.extractUsernameFromToken(token.replace("Bearer ", ""));
        validateAdminOrLibraryAdmin(adminUsername);
    ...
    /**
     * 获取组织标签树结构
     * ADMIN 和 LIBRARY_ADMIN 均可访问（上传对话框需要加载标签树）
     */
    ...
    /**
     * 验证用户是否为管理员或库管（ADMIN 或 LIBRARY_ADMIN 可通过）
     * 用于文档管理类操作
     */
    private User validateAdminOrLibraryAdmin(String username) {
        if (username == null || username.isEmpty()) {
            throw new CustomException("Invalid token", HttpStatus.UNAUTHORIZED);
        }
        ...
        if (user.getRole() != User.Role.ADMIN && user.getRole() != User.Role.LIBRARY_ADMIN) {
            throw new CustomException("Unauthorized access: Admin or Library Admin role required", HttpStatus.FORBIDDEN);
        }
    ...
    /**
     * 检查用户是否有管理权限（ADMIN 或 LIBRARY_ADMIN）
     */
    private boolean hasAdminRole(User user) {
        return user.getRole() == User.Role.ADMIN || user.getRole() == User.Role.LIBRARY_ADMIN;
    }
...
/**
 * 管理员用户请求体（role 可选，默认 ADMIN）
 */
record AdminUserRequest(String username, String password, String role) {
    AdminUserRequest(String username, String password) {
