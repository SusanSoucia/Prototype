...
import com.group45.backend.service.FileTypeValidationService;
import com.group45.backend.service.ParseService;
import com.group45.backend.service.UploadService;
...
public class UploadController {
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
        ...
        try {
            ...
            if (!userService.isAdminUser(userId)) {
                LogUtils.logUserOperation(userId, "UPLOAD_CHUNK", fileName, "FAILED_NON_ADMIN_UPLOAD");
                monitor.end("分片上传失败: 非管理员用户不允许上传文件");

                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("code", HttpStatus.FORBIDDEN.value());
                errorResponse.put("message", "仅管理员可以上传文件到知识库");
    ...
    public ResponseEntity<Map<String, Object>> mergeFile(
        ...
        try {
            ...
            if (fileUpload.getStatus() == FileUpload.STATUS_COMPLETED) {
                ...
                monitor.end("文件已完成合并");
                return buildAlreadyMergedResponse(request.fileMd5(), request.fileName());
            }
    ...
    private ResponseEntity<Map<String, Object>> buildAlreadyMergedResponse(String fileMd5, String fileName) throws Exception {
        Map<String, Object> data = new HashMap<>();
        data.put("objectUrl", uploadService.generateMergedObjectUrl(fileMd5, fileName));
