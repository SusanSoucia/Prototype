...
public class HybridSearchService {
    ...
    /**
     * 跨多个 ES 索引进行权限搜索，合并并按分数排序返回 topK 结果。
     * <p>
     * 当用户有权访问多个知识库（ES 索引）时使用此方法。
     * 对每个索引分别执行带权限的混合搜索，将结果合并后按相关性分数降序排列。
     *
     * @param query      搜索查询
     * @param userId     用户 ID
     * @param topK       返回结果数
     * @param indexNames ES 索引名列表
     * @return 合并排序后的搜索结果
     */
    public List<SearchResult> searchAcrossIndices(String query, String userId, int topK, List<String> indexNames) {
        if (indexNames == null || indexNames.isEmpty()) {
            logger.warn("索引名列表为空，回退到默认索引搜索");
            return searchWithPermission(query, userId, topK);
        }

        if (indexNames.size() == 1) {
            // 单索引：直接使用现有逻辑（此处使用默认索引方法）
            return searchWithPermission(query, userId, topK);
        }

        logger.info("跨 {} 个索引执行搜索: {}", indexNames.size(), indexNames);

        // 对每个索引分别搜索，然后合并结果
        // 注意：多索引场景下，每个索引返回 topK 条，合并后重新排序取 topK
        int perIndexTopK = Math.max(topK, 20);
        List<SearchResult> allResults = new ArrayList<>();

        for (String indexName : indexNames) {
            try {
                // 在每个索引上执行相同的混合搜索逻辑
                List<SearchResult> indexResults = searchSingleIndex(query, userId, perIndexTopK, indexName);
                allResults.addAll(indexResults);
            } catch (Exception e) {
                logger.warn("索引 {} 搜索失败，跳过: {}", indexName, e.getMessage());
            }
        }

        // 去重（按 fileMd5 + chunkId），保留最高分
        ...
        for (SearchResult r : allResults) {
            ...
            if (existing == null || r.getScore() > existing.getScore()) {
                bestByChunk.put(key, r);
            }
        }

        // 按分数降序排列，取 topK
        List<SearchResult> merged = bestByChunk.values().stream()
                .sorted(Comparator.comparing(SearchResult::getScore).reversed())
                .limit(topK)
                .toList();

        logger.info("多索引搜索完成: {} 个索引 → {} 条结果 → 去重合并后 {} 条", indexNames.size(), allResults.size(), merged.size());
        return merged;
    }

    /**
     * 在单个指定的 ES 索引上执行带权限的混合搜索。
     */
    private List<SearchResult> searchSingleIndex(String query, String userId, int topK, String indexName) {
        try {
            List<String> userEffectiveTags = getUserEffectiveOrgTags(userId);
            String userDbId = getUserDbId(userId);
            final List<Float> queryVector = embedToVectorList(query, userId);
            ...
            if (queryVector == null) {
                logger.warn("向量生成失败，仅使用文本匹配搜索索引 {}", indexName);
                return textOnlySearchWithPermission(query, userDbId, userEffectiveTags, topK);
            }
            ...
            logger.error("索引 {} 搜索失败: {}", indexName, e.getMessage(), e);
            return Collections.emptyList();
        }
    }

    private java.util.Comparator<SearchResult> scoreComparator() {
        return java.util.Comparator.comparing(SearchResult::getScore);
    }
}
