...
import cowSvgRaw from '@/assets/svg-icon/cow.svg?raw';
...
function getLogoSvg() {
  // Remove fixed width/height so the SVG adapts to its container
  return cowSvgRaw.replace(/width="[^"]*"/, 'width="100%"').replace(/height="[^"]*"/, 'height="100%"');
}
