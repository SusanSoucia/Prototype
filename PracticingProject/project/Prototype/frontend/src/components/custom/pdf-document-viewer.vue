...
function resolveBlobMimeType(blob: Blob, fileName?: string): string {
  if (blob.type && blob.type !== 'application/octet-stream') {
    return blob.type;
  }
  if (!fileName) return blob.type || 'application/octet-stream';
  ...
  return mimeMap[ext] || blob.type || 'application/octet-stream';
}
...
async function openInNewTab() {
  ...
  try {
    // Fetch as blob first to avoid Content-Disposition: attachment forcing download
    const response = await fetch(targetUrl);
    const rawBlob = await response.blob();
    // Ensure correct MIME type so the browser renders the blob instead of downloading it
    const mimeType = resolveBlobMimeType(rawBlob, props.fileName);
    ...
    // Fallback: open the original presigned URL directly
...
async function loadDocument(url: string) {
  ...
  // 跨域非 API URL（如 MinIO 预签名链接）先 fetch 转 Blob URL，避免 CORS 问题
  let resolvedUrl = url;
  if (/^https?:\/\//i.test(url) && !url.includes('/api/v1/documents/page-preview')) {
    try {
      const blob = await fetch(url).then(r => r.blob());
      resolvedUrl = URL.createObjectURL(blob);
    } catch {
      renderError.value = 'PDF 加载失败：无法获取文件，请检查对象存储是否可达。';
  ...
  try {
    const shouldAttachAuthHeaders = !/^https?:\/\//i.test(resolvedUrl) || resolvedUrl.includes('/api/v1/documents/page-preview');
    ...
    // Blob URL 用完后释放
    if (resolvedUrl !== url) URL.revokeObjectURL(resolvedUrl);
...
function applyHighlight() {
  ...
  textLayerTask.textContentItemsStr.forEach((item: string, index: number) => {
