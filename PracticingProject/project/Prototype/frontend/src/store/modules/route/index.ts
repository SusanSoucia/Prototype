...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  function initStaticAuthRoute() {
    ...
    if (authStore.isStaticSuper) {
      ...
      // Dynamically set home route to the first available auth route
      // (needed when VITE_ROUTE_HOME is not accessible to the current role)
      const firstAvailable = filteredAuthRoutes.find(r => !r.meta?.hideInMenu);
      if (firstAvailable) {
        setRouteHome(firstAvailable.name as LastLevelRouteKey);
        handleUpdateRootRouteRedirect(firstAvailable.name as LastLevelRouteKey);
      }
    }
