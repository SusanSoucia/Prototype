...
export const useAuthStore = defineStore(SetupStoreId.Auth, () => {
  ...
  /** Is library admin (库管) — only true for LIBRARY_ADMIN role, not ADMIN */
  const isLibraryAdmin = computed(() => userInfo.role === 'LIBRARY_ADMIN');
  ...
  /** Dev mock: return fake user info so the app can be tested without a running backend */
  function getDevMockUserInfo(): Api.Auth.UserInfo | null {
    const devMockRole = import.meta.env.VITE_DEV_MOCK_ROLE as string | undefined;
    if (!import.meta.env.DEV || !devMockRole) return null;
    if (getToken() !== 'dev-mock-token') return null;

    const validRoles = ['ADMIN', 'LIBRARY_ADMIN', 'USER'];
    const role = validRoles.includes(devMockRole) ? devMockRole : 'ADMIN';

    /**
     * Mock 用户数据 — 反映组织标签架构：
     * - ADMIN 标签自身代表超管身份，其子标签是各个库管（知识库）
     * - LIBRARY_ADMIN 只持有一个子标签 = 一个知识库
     * - USER 持有 USER 树下的普通用户标签
     */
    const mockUsers: Record<string, Api.Auth.UserInfo> = {
      ADMIN: {
        id: 1,
        userName: 'admin',
        role: 'ADMIN',
  ...
  async function getUserInfo() {
    // Dev mock shortcut — bypass backend API entirely
    const mockUser = getDevMockUserInfo();
    if (mockUser) {
      Object.assign(userInfo, mockUser);
      console.log('[dev-mock] Injected user info:', mockUser);
      return true;
    }
