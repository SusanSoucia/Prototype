...
import { SetupStoreId } from '@/enum/index';
...
export const useChatStore = defineStore(SetupStoreId.Chat, () => {
  ...
  function upsertGenerationSnapshot(snapshot: Api.Chat.GenerationSnapshot | null) {
    ...
    if (assistantIndex >= 0) {
      ...
      if (snapshot.toolEvents && snapshot.toolEvents.length > 0) {
        assistant.toolEvents = snapshot.toolEvents;
      }
      return;
    }
