...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
  ...
  const {
    ...
    transformer: res => {
      ...
      // Structure 1: { number, size, totalElements, data } (standard)
      // Structure 2: { records, current, size, total } (from fetchGetOrgTagList)
      // Structure 3: Array directly (when request interceptor extracts data.data)
      ...
      if (Array.isArray(res.data)) {
        ...
        // Structure 2: From fetchGetOrgTagList
        ...
        records = (res.data?.data || res.data?.content || []) as any[];
        pageNum = number;
