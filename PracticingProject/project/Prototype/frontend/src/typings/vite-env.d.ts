...
declare namespace Env {
  ...
  interface ImportMeta extends ImportMetaEnv {
    ...
    /**
     * Whether to auto login when a token exists in localStorage
     *
     * Set to N to require manual login each time the app starts
     */
    readonly VITE_AUTO_LOGIN?: CommonType.YesOrNo;
    /** Dev only: mock role to skip login (ADMIN | LIBRARY_ADMIN | USER). Leave empty to disable. */
    readonly VITE_DEV_MOCK_ROLE?: string;
    /** Force enable mock server in dev mode (Y=on). When enabled, uses mock/ data instead of backend. */
