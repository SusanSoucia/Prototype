        ...
        try {
            ...
            http.csrf(csrf -> csrf.disable())
                    ...
                    .authorizeHttpRequests(authorize -> authorize
                            ...
                            // 文件上传、解析、文档管理写操作仅管理员可用
                            .requestMatchers("/api/v1/upload/**", "/api/v1/parse").hasRole("ADMIN")
                            ...
                            .requestMatchers("/api/v1/documents/uploads").hasRole("ADMIN")
                            // 知识库问答引用查看接口 - 普通用户和管理员都可访问
                            .requestMatchers(
                                    "/api/v1/documents/accessible",
                                    "/api/v1/documents/preview",
                                    "/api/v1/documents/page-preview",
                                    "/api/v1/documents/reference-detail"
                            ).hasAnyRole("USER", "ADMIN")
                            // 登录用户基础信息、用量与登出接口
                            .requestMatchers(
                                    "/api/v1/users/me",
                                    "/api/v1/users/usage",
                                    "/api/v1/users/token-records",
                            ...
                            // 用户组织标签管理接口
                            .requestMatchers("/api/v1/users/primary-org", "/api/v1/users/upload-orgs").hasRole("ADMIN")
                            // 未明确放行的接口默认管理员可访问，避免普通用户拿到上传或管理能力
