...
import com.group45.backend.service.UsageDashboardService;
...
public class AdminController {
    ...
    @PutMapping("/documents/{fileMd5}/org-tag")
    public ResponseEntity<?> updateDocumentOrgTag(
            @RequestHeader("Authorization") String token,
            @PathVariable String fileMd5,
            @RequestBody DocumentOrgTagUpdateRequest request) {
        String adminUsername = null;
        ...
        try {
            ...
            return ResponseEntity.ok(Map.of(
                    "code", 200,
                    "message", "文档组织标签更新成功",
                    "data", documentService.updateDocumentOrgTag(fileMd5, request.orgTag())
            ));
        } catch (CustomException e) {
            LogUtils.logBusinessError("ADMIN_UPDATE_DOCUMENT_ORG_TAG", adminUsername, "更新文档组织标签失败: %s", e, e.getMessage());
            return ResponseEntity.status(e.getStatus()).body(Map.of("code", e.getStatus().value(), "message", e.getMessage()));
        } catch (Exception e) {
            LogUtils.logBusinessError("ADMIN_UPDATE_DOCUMENT_ORG_TAG", adminUsername, "更新文档组织标签异常: %s", e, e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("code", 500, "message", "更新文档组织标签失败: " + e.getMessage()));
        }
    }

    @PutMapping("/documents/{fileMd5}/visibility")
    public ResponseEntity<?> updateDocumentVisibility(
            @RequestHeader("Authorization") String token,
            @PathVariable String fileMd5,
            @RequestBody DocumentVisibilityUpdateRequest request) {
        String adminUsername = null;
        ...
        try {
            ...
            return ResponseEntity.ok(Map.of(
                    "code", 200,
                    "message", "文档可见性更新成功",
                    "data", documentService.updateDocumentVisibility(fileMd5, request.isPublic())
            ));
        } catch (CustomException e) {
            LogUtils.logBusinessError("ADMIN_UPDATE_DOCUMENT_VISIBILITY", adminUsername, "更新文档可见性失败: %s", e, e.getMessage());
            return ResponseEntity.status(e.getStatus()).body(Map.of("code", e.getStatus().value(), "message", e.getMessage()));
        } catch (Exception e) {
            LogUtils.logBusinessError("ADMIN_UPDATE_DOCUMENT_VISIBILITY", adminUsername, "更新文档可见性异常: %s", e, e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("code", 500, "message", "更新文档可见性失败: " + e.getMessage()));
        }
    }
...
record DocumentOrgTagUpdateRequest(String orgTag) {}

record DocumentVisibilityUpdateRequest(boolean isPublic) {}
