...
public class OrgTagCacheService {
    ...
    /**
     * 获取文档访问用的标签集合：用户直接拥有的标签及所有子标签。
     */
    public List<String> getDocumentAccessOrgTags(List<String> userTags) {
        if (userTags == null || userTags.isEmpty()) {
            return Collections.emptyList();
        }

        Set<String> accessTags = new HashSet<>();
        for (String tagId : userTags) {
            if (tagId == null || tagId.isBlank()) {
                continue;
            }
            String normalizedTagId = tagId.trim();
            accessTags.add(normalizedTagId);
            collectChildTags(normalizedTagId, accessTags);
        }
        return new ArrayList<>(accessTags);
    }
    ...
    private void collectChildTags(String tagId, Set<String> result) {
        try {
            List<OrganizationTag> children = organizationTagRepository.findByParentTag(tagId);
            for (OrganizationTag child : children) {
                if (child == null || child.getTagId() == null || result.contains(child.getTagId())) {
                    continue;
                }
                result.add(child.getTagId());
                collectChildTags(child.getTagId(), result);
            }
        } catch (Exception e) {
            logger.error("Error collecting child tags for tag: {}", tagId, e);
        }
    }
