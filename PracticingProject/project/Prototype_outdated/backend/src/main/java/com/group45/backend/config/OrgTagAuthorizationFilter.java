...
import com.group45.backend.service.OrgTagCacheService;
...
public class OrgTagAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            String requestUserId = jwtUtils.extractUserIdFromToken(token);
            String role = jwtUtils.extractRoleFromToken(token);
            
            // 如果是资源拥有者，直接放行
            if ((username != null && username.equals(resourceInfo.getOwner()))
                    || (requestUserId != null && requestUserId.equals(resourceInfo.getOwner()))) {
    ...
    private boolean isUserAuthorized(String userOrgTags, String resourceOrgTag) {
        // 将用户的组织标签字符串转换为集合
        java.util.List<String> userTags = Arrays.stream(userOrgTags.split(","))
                .map(String::trim)
                .filter(tag -> !tag.isEmpty())
                .toList();
        
        // 用户拥有父标签时，可访问该标签及其所有子标签下的文档。
        return orgTagCacheService.getDocumentAccessOrgTags(userTags).contains(resourceOrgTag);
    }
