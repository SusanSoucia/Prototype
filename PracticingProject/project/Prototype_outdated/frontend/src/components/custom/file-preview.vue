...
function resolveFileAccessUrl(url: string) {
  ...
  if (url.startsWith('/api/')) {
    if (serviceBaseUrl.startsWith('/proxy-')) {
      // 保留 /api/v1 前缀，代理 rewrite 去掉 /proxy-default 后路径正确转发到后端
  ...
  try {
    ...
    if (props.fileMd5) {
      ...
      if (requestError) {
        ...
        await triggerDownload(data.downloadUrl, data.fileName);
      }
    } else {
      // 降级：使用文件名下载（向后兼容）
      const { error: requestError, data } = await request<{
...
// 下载文件（处理跨域预签名 URL 的文件名问题）
async function triggerDownload(downloadUrl: string, fileName: string) {
  try {
    const isCrossOrigin = /^https?:\/\//i.test(downloadUrl) &&
      new URL(downloadUrl).origin !== window.location.origin;

    if (isCrossOrigin) {
      // 跨域 URL：先 fetch 再通过 Blob 下载，确保文件名正确
...
async function openPreviewInNewTab() {
  ...
  try {
    ...
    if (previewType.value === 'pdf' && displayPage.value) {
      window.open(`${blobUrl}#page=${displayPage.value}`, '_blank', 'noopener,noreferrer');
    } else {
